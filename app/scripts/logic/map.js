...
export function makeIsland(x, y, w, h) {
  ...
  for (var i = 0; i < h; i++) {
    for (var j = 0; j < w; j++) {
      ...
      if (i == 0 || i == h - 1) tile.terrainLevel = SHALLOW;

      if (j != 0 && j != w - 1 && (i == 1 || i == h - 2)) tile.terrainLevel = COAST;

      if (j == 0 || j == w - 1) tile.terrainLevel = SHALLOW;

      if (i != 0 && i != h - 1 && (j == 1 || j == w - 2)) tile.terrainLevel = COAST;

      if (tile.terrainLevel >= COAST) tile.islandId = newIsland.id;
...
export function makeMountain(x, y) {
  for (var i = 0; i < 6; i++) {
    for (var j = 0; j < 6; j++) {
      ...
      if (tile.buildingData != null) tile.buildingData.remove();

      if (i == 0 || i == 5 || j == 0 || j == 5) tile.terrainLevel = HILLSIDE;
      else tile.terrainLevel = MOUTAIN;
