...
var Storage = Class.extend(function () {
  ...
  this.add = function (type, quantity) {
    ...
    if (this.slots[type] == undefined) this.slots[type] = new Slot(type, quantity);
    else this.slots[type].quantity += quantity;
  };
  ...
  this.remove = function (type, quantity) {
    ...
    if (this.slots[type] == undefined) return false;

    var wasQuantity = this.slots[type].quantity;

    if (quantity == Infinity) this.slots[type].quantity = 0;
    else {
      if (quantity > this.slots[type].quantity) return this.remove(type, Infinity);
